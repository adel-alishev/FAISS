...
def use_offline_recognition():
    ...
    try:
        ...
        if not os.path.exists("models/vosk-model-small-" + assistant.speech_language + "-0.22"):
        ...
        model = Model("models/vosk-model-small-" + assistant.speech_language + "-0.22")
...
def get_translation(*args: tuple):
    ...
    try:
        ...
        # если язык речи ассистента и родной язык пользователя одинаковы, то перевод выполяется на изучаемый язык
            ...
            translation_result = google_translator.translate(text=search_term,  # что перевести
            ...
            #print("По-английски {} будет как".format(search_term))
